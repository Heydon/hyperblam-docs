...
Instead, we can apply swing using the `<sequencer-blam>` element's `swing` prop. This pushes every other beat up against the next. By default, these shifted beats are moved *one third of one quarter-beat* to the right. Effectively, this makes them the third beat in *a inaginary triplet spread over two quarter beats (*or one half beat).
...
So, while you can apply `choke` as a Boolean attribute and get `0.333` of swing, you can adjust the proportion to taste by supplying an explicit value. This is in keeping with classic hardware drum machines and grooveboxes, which typically offer variable swing. 
