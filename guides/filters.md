...
A phaser effect messes with phase on purpose. Using an *allpass* filter (a filter that does *not* alter freqency information), it creates a copy of the original signal. The phase relationship between the copy and the original can be changed over time. 
